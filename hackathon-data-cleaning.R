...
inTrain2 <- as.vector(createDataPartition(mi_comp_clean2[,46],p=0.8,list=FALSE, times=1))
dTrain2 <- mi_comp_clean2[inTrain2,]; dim(dTrain2)
dTest2 <- mi_comp_clean2[-inTrain2,]; dim(dTest2)

#gather response var into vectors
yTrain2 <- dTrain2[,46]
xTrain2 <- as.matrix(dTrain2[,-46])
yTest2 <- dTest2[,46]
xTest2 <- as.matrix(dTest2[,-46])
...
fit.lasso.b3 <- glmnet(xTrain, yTrain, alpha=0.5, standardize=TRUE, family="binomial")
plot(fit.lasso.b3, label=TRUE, xvar="lambda")


set.seed(777)
cv.lasso.b3 <- cv.glmnet(xTrain, yTrain, alpha=0.5, standardize=TRUE, family="binomial", nfolds=10)
plot(cv.lasso.b3)
...
#FIRST, look at the estimated probabilities, and then convert these into"TRUE" or "FALSE":
phat <- predict(cv.lasso.b3, newx=xTest[1:10,], s=cv.lasso.b3$lambda.1se, type="response")
yhat <- predict(cv.lasso.b3, newx=xTest[1:10,], s=cv.lasso.b3$lambda.1se, type="class")
cbind(phat, yhat, yTest3[1:10], unname(dTest3[1:10,]))

#SECOND,consider many ways to convert the estimated probabilities into TRUE or FALSE
phat <- predict(cv.lasso.b3, newx=xTest, s=cv.lasso.b3$lambda.1se, type="response")
#install.packages("ROCR")
library(ROCR)
perf <- ROCR::performance(ROCR::prediction(phat, yTest), "tpr", "fpr")
par(las=1)
...
fit.lasso.b4 <- glmnet(xTrain2, yTrain2, alpha=0.5, standardize=TRUE, family="binomial")
plot(fit.lasso.b4, label=TRUE, xvar="lambda")

set.seed(3201)
cv.lasso.b4 <- cv.glmnet(xTrain2, yTrain2, alpha=0.5, standardize=TRUE, family="binomial", nfolds=10)
plot(cv.lasso.b4)
...
#FIRST, look at the estimated probabilities, and then convert these into"TRUE" or "FALSE":
phat <- predict(cv.lasso.b4, newx=xTest2[1:10,], s=cv.lasso.b4$lambda.1se, type="response")
yhat <- predict(cv.lasso.b4, newx=xTest2[1:10,], s=cv.lasso.b4$lambda.1se, type="class")
cbind(phat, yhat, yTest2[1:10], unname(dTest2[1:10,]))

#SECOND,consider many ways to convert the estimated probabilities into TRUE or FALSE
phat <- predict(cv.lasso.b4, newx=xTest2, s=cv.lasso.b4$lambda.1se, type="response")
#install.packages("ROCR")
library(ROCR)
perf <- ROCR::performance(ROCR::prediction(phat, yTest2), "tpr", "fpr")
par(las=1)
